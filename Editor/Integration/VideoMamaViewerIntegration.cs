...
{
    /// <summary>
    /// Runs <c>alpha.videomama_hint</c> via <see cref="ProcessBackendClient"/>, then loads first frame + alpha
    /// into INPUT/OUTPUT panes and updates A/B comparison renderers.
    /// </summary>
    public sealed class VideoMamaViewerIntegration : IDisposable
    {
        ...
        public VideoMamaViewerIntegration(
        ...
        public void RequestVideoMamaForDefaultClip(QueueJobVm? queueRow = null)
        {
            if (!CorridorKeyDataPaths.TryGetDefaultTestClip(out var clipRoot, out var framesDir))
            {
                Debug.LogError("[CorridorKey] VideoMaMa: default test clip not found.");
            ...
            {
                Debug.LogError($"[CorridorKey] VideoMaMa: refusing clip_root outside project: {clipRoot}");
            ...
            var payload = new VideoMamaHintStdin
            {
                cmd = "alpha.videomama_hint",
            ...
            if (err != null)
            {
                ...
                Debug.LogError($"[CorridorKey] VideoMaMa: bridge send failed — {err}");
                return;
            }

            var status = _body.Q<Label>("viewer-shared-status-label");
            if (status != null)
                status.text = "VideoMaMa... (see Console)";
            Debug.Log($"[CorridorKey] VideoMaMa started (request_id={requestId}).");
        }
        ...
        void OnBridgeCommandDone(BridgeCommandDonePayload p)
        {
            ...
            {
                if (status != null)
                    status.text = "VideoMaMa failed";
                Debug.LogError($"[CorridorKey] VideoMaMa finished with error: {p.Summary}");
                return;
            }
            ...
            {
                Debug.LogWarning("[CorridorKey] VideoMaMa ok but could not resolve first frame / alpha paths.");
                if (status != null)
                    status.text = "Ready";
                return;
            }
            ...
            if (status != null)
                status.text = "Ready — VideoMaMa alpha ready (toggle A/B to compare)";
        }
        ...
        void ApplyDualViewTextures(string framePath, string alphaPath)
        {
            ...
            {
                Debug.LogWarning("[CorridorKey] Failed to load VideoMaMa preview textures from disk.");
                return;
            }
