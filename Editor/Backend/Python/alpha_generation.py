...
def _run_alpha_videomama_hint(
    request_id: str,
    clip_root: str,
    frames_dir: str,
    overwrite: bool,
) -> None:
    cmd_name = "alpha.videomama_hint"
    try:
        clip_root = os.path.abspath((clip_root or "").strip())
        frames_dir = os.path.abspath((frames_dir or "").strip())
        if not clip_root or not frames_dir:
        ...
        bridge_core._emit(
            {
                ...
                "logger": "unity_bridge",
                "message": (
                    f"run_videomama request_id={request_id}: clip_root={clip_root}; "
                    f"image_files_in_frames_dir={len(frame_files)}; overwrite={overwrite}"
                ),
            }
        )
        ...
        bridge_tmp_dir = os.path.join(clip_root, ".bridge_tmp")
        os.makedirs(bridge_tmp_dir, exist_ok=True)
        result_path = os.path.join(bridge_tmp_dir, f"corridorkey_videomama_result_{request_id}.json")
        status_path = os.path.join(bridge_tmp_dir, f"corridorkey_videomama_status_{request_id}.json")
        stderr_path = os.path.join(bridge_tmp_dir, f"corridorkey_videomama_stderr_{request_id}.log")
        session_log_path = os.path.join(bridge_tmp_dir, f"corridorkey_videomama_session_{request_id}.log")
        try:
            with open(stderr_path, "w", encoding="utf-8", buffering=1) as _sf:
                _sf.write(f"unity_bridge: videomama stderr (request_id={request_id})\n")
        except Exception:
            stderr_path = ""
        try:
            with open(session_log_path, "w", encoding="utf-8", buffering=1) as _lf:
        ...
        runner_path = os.path.join(os.path.dirname(__file__), "videomama_hint_runner.py")
        try:
            if os.path.exists(result_path):
                os.remove(result_path)
            if os.path.exists(status_path):
        ...
        if use_stderr_pipe and proc.stderr is not None:
            ...
            def _pump_videomama_stderr() -> None:
                try:
                    with open(stderr_path, "a", encoding="utf-8", buffering=1) as out:
                        for line in proc.stderr:
                            out.write(line)
            ...
            threading.Thread(target=_pump_videomama_stderr, daemon=True, name="videomama-stderr-pump").start()
        ...
        while True:
                            ...
                            bridge_core._emit(
                                {
                                    ...
                                    "level": "INFO",
                                    "logger": "unity_bridge",
                                    "message": f"VideoMaMa [{stage}]: {detail}" if detail else f"VideoMaMa [{stage}]",
                                }
                            )
                        sc = st.get("current")
                        stt = st.get("total")
            ...
            if os.path.isfile(result_path):
                with open(result_path, "r", encoding="utf-8") as rf:
                    result = json.load(rf)
                if not result.get("ok", False):
                    raise RuntimeError(str(result.get("message") or "videomama runner failed"))
                alpha_count = int(result.get("alpha_count", curr))
                bridge_core._emit(
                    {
                        "type": "diag_result",
                        ...
                        "diag": "videomama_hint",
                        "ok": True,
                        "summary": str(result.get("message") or f"VideoMaMa wrote {alpha_count} alpha hint frame(s)"),
                    }
                )
                break
            ...
            if proc.poll() is not None:
                ...
                if code != 0:
                    if os.path.isfile(result_path):
                        with open(result_path, "r", encoding="utf-8") as rf:
                            result = json.load(rf)
                        raise RuntimeError(str(result.get("message") or f"videomama runner exited with code {code}"))
                    raise RuntimeError(f"videomama runner exited with code {code} without result file")
                spin = 0
                while spin < 10 and not os.path.isfile(result_path):
                    threading.Event().wait(0.2)
                    spin += 1
                if os.path.isfile(result_path):
                    break
                raise RuntimeError("videomama runner exited successfully but result file not found")
                ...
                bridge_core._emit(
                    {
                        ...
                        "level": "INFO",
                        "logger": "unity_bridge",
                        "message": f"videomama still running {int(waited)}s; see {session_log_path}",
                    }
                )
            if waited >= timeout_s:
                proc.kill()
                raise RuntimeError(f"run_videomama timed out after {int(timeout_s)}s")

        alpha_count = len([n for n in os.listdir(alpha_dir) if n.lower().endswith(".png")]) if os.path.isdir(alpha_dir) else 0
        media_processing._update_clip_json_with_alpha(clip_root, alpha_dir, alpha_count, status="videomama_hint_generated")
        bridge_core._emit_done(cmd_name, request_id, ok=True, summary=f"alpha_hint_frames={alpha_count}")
    except Exception as exc:  # noqa: BLE001
        bridge_core._emit({"type": "error", "message": str(exc)})
        bridge_core._emit_done(cmd_name, request_id, ok=False, summary=str(exc))
